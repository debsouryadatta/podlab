...
image = (modal.Image.from_registry( # 1. Create an image
    "nvidia/cuda:12.4.0-devel-ubuntu22.04", add_python="3.12") # 2. Add cuda and python
    .apt_install(["ffmpeg", "libgl1-mesa-glx", "wget", "libcudnn8", "libcudnn8-dev"]) # 3. Install ffmpeg, libgl1-mesa-glx, wget, libcudnn8, libcudnn8-dev
    .pip_install_from_requirements("requirements.txt") # 4. Install the requirements
    .run_commands(["mkdir -p /usr/share/fonts/truetype/custom", # 5. Create a directory for the fonts
                   "wget -O /usr/share/fonts/truetype/custom/Anton-Regular.ttf https://github.com/google/fonts/raw/main/ofl/anton/Anton-Regular.ttf", # 6. Download the font
                   "fc-cache -f -v"])
    .add_local_dir("asd", "/asd", copy=True)) # 7. Add the local directory

app = modal.App("podlab", image=image)

# this volume gets stored in the cloud and is shared between the containers
volume = modal.Volume.from_name(
    "podlab-model-cache", create_if_missing=True
)
# the volume is mounted to the container at the mount_path when the container is created
...
def create_vertical_video(tracks, scores, pyframes_path, pyavi_path, audio_path, output_path, framerate=25):
    ...
    # merge a video file with an audio file into a single output video
...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    s3_client.upload_file(subtitle_output_path, "podlab-ai", output_s3_key)

@app.cls(gpu="L40S", retries=0, scaledown_window=20, secrets=[modal.Secret.from_name("podlab")], volumes={mount_path: volume})
class Podlab:
    @modal.enter()
    def load_model(self):
        # 1. whisperx_model - main transcription model
        # 2. alignment_model - model that provides precise word-level timestamps by aligning the transcribed text with the original audio
    ...
    def transcribe_video(self, base_dir: str, video_path: str) -> str:
        ...
        extract_cmd = f"ffmpeg -i {video_path} -vn -acodec pcm_s16le -ar 16000 -ac 1 {audio_path}" # Extract audio from video
        ...
        result = self.whisperx_model.transcribe(audio, batch_size=16) # batch_size depends on gpu
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("podlab-ai", s3_key, str(video_path))
...
def main():
    ...
    url = podlab.process_video.web_url
